...
class_servo_motor_t *servo_motor = &servo_motor_template.class;
...
void ControllerMotor_Init(void)
{
    ...
    */
    ...
    servo_motor->position.p = 4.0;
    servo_motor->position.i = 0.02;
    servo_motor->position.d = 100.0;

    // Speed PID default values
    servo_motor->speed.p = 0.1;
    servo_motor->speed.i = 0.1;
    servo_motor->speed.d = 0.0;
    ...
    if (deltatime >= ASSERV_PERIOD)
    {
        ...
        else
        {
            ...
            if (servo_motor->mode.mode_angular_position || servo_motor->mode.mode_linear_position)
            {
                ...
                if (errAngleSum > 100.0)
                    errAngleSum = 100;
                anglePower = (errAngle * servo_motor->position.p) + (errAngleSum * servo_motor->position.i) + (dErrAngle * servo_motor->position.d); // raw PID command
            ...
            if (servo_motor->mode.mode_angular_speed || servo_motor->mode.mode_linear_speed)
            {
                ...
                if (errSpeedSum > 100.0)
                    errSpeedSum = 100;
                speedPower = ((errSpeed * servo_motor->speed.p) + (errSpeedSum * servo_motor->speed.i) + (dErrSpeed * servo_motor->speed.d)); // raw PID command
