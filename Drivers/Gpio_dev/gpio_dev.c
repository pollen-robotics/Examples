...
class_state_t *gpio[GPIO_NB];

template_voltage_t analog_template[ANALOG_NB];
class_voltage_t *analog[ANALOG_NB];
...
void GpioDev_Init(void)
{
    ...
    */
    ...
    // Link user varaibles to template class.
    for (uint8_t i = 0; i < ANALOG_NB; i++)
    {
        analog[i] = &analog_template[i].class;
    }
    // Class configuration
